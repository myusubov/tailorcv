...
- **Rule**: Backend detected-area rules are dispatched from `detected-area-rules/backend/backend-area-rules.ts`. NestJS, Django, and Spring Boot run first with implemented path rules; ASP.NET Core, Laravel, and Rails remain scaffolded before the generic backend fallback.
...
- **Rule**: Spring Boot detector confidence coverage uses official/simple, Gradle-backed web, JHipster-style REST, config-backed web, Kotlin, mixed Java/Kotlin, monorepo-isolation, repeated-signal, isolated-signal, weak-cluster, and documentation-sample fixtures through the public analyzer output.
