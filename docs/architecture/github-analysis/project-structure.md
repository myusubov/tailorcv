...
- **Rule**: Backend detected-area rules are dispatched from `detected-area-rules/backend/backend-area-rules.ts`. NestJS, Django, Spring Boot, and ASP.NET Core run first with implemented path rules; Laravel and Rails remain scaffolded before the generic backend fallback.
