...
- **Rule**: Docker containerization matching currently recognizes normalized Dockerfile variants, `.dockerignore` variants, Compose files such as `compose.yaml`, `compose.prod.yaml`, `docker-compose.yml`, and `docker-compose.prod.yml`, and `.devcontainer/devcontainer.json`; the Docker detector gate still controls whether those support signals emit a `Containerization` area.
