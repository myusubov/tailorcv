...
| `apps/backend/src/services/github-analysis/project-structure/detected-area-rules/declarative-area-rule-engine.ts` | Generic schema-driven detector runner used by 17 of the framework, database, containerization, and test detectors; supports `gateBlocker` shape checks and `competingProofSchemas` vetoes (see the 2026-08-27 changelog entry). Seven detectors remain permanently hand-written -- see Risks & Mitigations below. | Changing shared detector matching/gating/veto behavior |
...
- [x] Declarative area-rule engine for framework detectors (17 detectors migrated across two waves with gate and competing-proof support; Spring Boot, Laravel, Rails, ASP.NET Core, React, Static frontend, and Express remain permanently hand-written, see the 2026-08-27 changelog entry)
...
| Seven detectors (Spring Boot, Laravel, Rails, ASP.NET Core, React, Static frontend, Express) cannot be expressed in the declarative schema as it exists today -- dynamic per-owner related technology, gating on the shared candidate map instead of raw file evidence, per-entry exclusion predicates, and a two-independent-OR-groups condition shape are all unsupported | Keep these detectors hand-written; only extend `applyDeclarativeAreaDetector` if a concrete need arises, see the 2026-08-27 changelog entry. |
