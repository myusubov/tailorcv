...
- **Rule**: Docker containerization matching currently recognizes basename-only Dockerfile variants, `.dockerignore` variants, Compose files such as `compose.yaml`, `compose.prod.yaml`, `docker-compose.yml`, and `docker-compose.prod.yml`, Docker Bake files such as `docker-bake.hcl` and `docker-bake.override.json`, and path-scoped `.devcontainer/devcontainer.json`; the Docker detector gate still controls whether those support signals emit a `Containerization` area.
- **Rule**: Docker containerization owner resolution points at the repo area being containerized: monorepo paths resolve to `apps/*`, `services/*`, `packages/*`, or `libs/*`; root and generic config folders such as `docker/`, `deploy/`, and `.devcontainer/` resolve to `.`; simple local owners such as `backend/Dockerfile` fall back to the parent folder.
