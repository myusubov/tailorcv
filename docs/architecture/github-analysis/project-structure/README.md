...
| `apps/backend/src/services/github-analysis/project-structure/detected-area-rules/declarative-area-rule-engine.ts` | Generic schema-driven detector runner used by every framework, database, containerization, and CI/CD detector (the Jest detector was removed with the Test-suite category, the shared-package detector on 2026-09-08; the ASP.NET Core (2026-08-29) and NestJS (2026-09-04) conversions leave none hand-written); supports `gateBlocker` shape checks with nested `and`/`or` condition nodes, `competingProofSchemas` file-evidence vetoes, a `checkForExistingCandidate` shared-candidate-map veto, a `dynamicRelatedTechMap` per-signal related-technology union (see the 2026-08-27, 2026-08-28, and 2026-08-29 changelog entries), and an optional `ownerAdapter` -- an anchor-signal two-pass resolver for framework/tool shapes, or a plain per-entry resolver as containerization uses. | Changing shared detector matching/gating/veto behavior |
| `apps/backend/src/services/github-analysis/project-structure/detected-area-rules/owner-adapters/`                 | Per-shape owner resolvers passed to the engine as `ownerAdapter`. `resolve-unit-root-owner.ts` anchors on the framework or tool config file at the unit root and is shared by fifteen detectors (Next.js, Nuxt, Angular, Astro, React Router, SvelteKit, Django, ASP.NET Core, Laravel, Prisma, Drizzle, SQLAlchemy, Sequelize, Knex, Jenkins) -- see [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md). `resolve-container-root-owner.ts` is the Docker and Podman/OCI detectors' shared non-anchor adapter: an infra-folder denylist plus a one-directory-deep rule over the widened generic resolver, with a second denylist (`PODMAN_OCI_NON_UNIT_TOP_LEVEL_DIRECTORIES`) and a Quadlet/`Containerfile` basename matcher for the Podman/OCI arm -- see [ADR 0004](adr/0004-containerization-owner-resolution.md).                                                                                                                                                                                                                                                                          | Adding an owner adapter for a new framework or tool shape      |
...
│   │   ├── resolve-container-root-owner.ts
│   │   ├── resolve-container-root-owner.test.ts
│   │   └── resolve-container-root-owner.podman-oci.test.ts
│   ├── backend/
...
│   ├── ci-cd/
│   │   ├── ci-cd-area-rules.ts
│   │   ├── github-actions-ci-cd-area-rules.ts
│   │   ├── gitlab-ci-cd-area-rules.ts
│   │   ├── circleci-ci-cd-area-rules.ts
│   │   ├── jenkins-ci-cd-area-rules.ts
│   │   ├── azure-pipelines-ci-cd-area-rules.ts
│   │   ├── buildkite-ci-cd-area-rules.ts
│   │   ├── travis-ci-cd-area-rules.ts
│   │   ├── bitbucket-ci-cd-area-rules.ts
│   │   ├── drone-woodpecker-ci-cd-area-rules.ts
│   │   ├── appveyor-ci-cd-area-rules.ts
│   │   └── teamcity-ci-cd-area-rules.ts
│   └── frontend/
...
- **Rule**: Root-level app or config evidence uses `path: "."`; monorepo app evidence uses owners such as `apps/frontend` or `apps/backend`.
...
- **Rule**: The declarative engine (`detected-area-rules/declarative-area-rule-engine.ts`) resolves every signal's owner through the generic `ownerPathForApplicationArea` unless the detector passes an `ownerAdapter`. With an anchor-based adapter, the engine evaluates anchor schemas (`isAnchorSignal: true`) first, collects each anchor signal's resolved owner into an `anchorOwners` set, then resolves non-anchor signals against that completed set in a second pass; a detector with no anchor schema just has its adapter called once per matched entry. Two adapters exist:
  - `detected-area-rules/owner-adapters/resolve-unit-root-owner.ts`, passed by fifteen detectors -- Next.js, Nuxt, Angular, Astro, React Router, SvelteKit (frontend); Django, ASP.NET Core, Laravel (backend); Prisma, Drizzle, SQLAlchemy, Sequelize, Knex (database); Jenkins (CI/CD). An anchor signal is the framework or tool config file that sits at the unit root by that tool's contract (`next.config.*`, `nuxt.config.*`, `angular.json`, `astro.config.*`, `react-router.config.*`, `svelte.config.*`, `manage.py`, `*.csproj`, `artisan`, `prisma.config.*`, `drizzle.config.*`, `alembic.ini`, `.sequelizerc`, `knexfile.*`, `Jenkinsfile`); it resolves to the directory holding that file. A non-anchor signal resolves to the longest `anchorOwners` entry that encloses it (a path with no directory segment resolves to `.`), otherwise it falls back to `ownerPathForApplicationArea`. Jenkins is the one exception to "sits at the unit root": a `Jenkinsfile` can live at any depth (a root single-pipeline repo, or one per service in a multibranch monorepo), so every matched `Jenkinsfile` is itself an anchor and resolves to its own containing directory -- the only CI/CD provider that can emit more than one `CI/CD workflows` candidate per repository. See [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md) (extended 2026-09-04, 2026-09-11).
  - `detected-area-rules/owner-adapters/resolve-container-root-owner.ts`, passed by the Docker and Podman/OCI detectors with no anchor schema. A bare root-level file resolves to `.`; a container file one directory deep resolves to that directory unless the directory is a known infra/tooling/workspace-container name, in which case `.`; every deeper path delegates to `ownerPathForApplicationArea` with the containerization roots added as workspace roots for that call. Docker file names (`Dockerfile`/`*.dockerfile` variants, `.dockerignore`, Compose, Bake) check `NON_UNIT_TOP_LEVEL_DIRECTORIES` and add `products`/`plugins`/`providers`; Quadlet unit and `Containerfile`/`.containerignore` names check the wider `PODMAN_OCI_NON_UNIT_TOP_LEVEL_DIRECTORIES` (adds `systemd`, `containers`, `.quadlet`, `quadlet`, `quadlets`, `kube`, `sysadmin`) and also add `quadlet`/`quadlets` as collection roots, so `quadlet/<svc>/<svc>.container` resolves to `quadlet/<svc>`. See [ADR 0004](adr/0004-containerization-owner-resolution.md) (extended 2026-09-10).

  Both are the bounded, per-shape exception to the 2026-08-24 generic-resolver rules below.
...
- **Rule (2026-09-08):** There is no `Shared package` detected-area detector. The `detected-area-rules/shared-package/` modules and their `addSharedPackageAreas` dispatch were removed (low-signal output -- reusable-package path shapes overlapped too heavily with ordinary monorepo packages); see the changelog. `'Shared package'` remains in the `DetectedAreaName` union and priority orderings but is never emitted.
- **Rule**: Containerization detected-area rules are dispatched from `detected-area-rules/containerization/containerization-area-rules.ts`. Runtime-specific containerization detectors live in separate modules: Docker and the Podman/OCI combination gate are implemented. Podman/OCI collects basename-only Quadlet unit, `Containerfile`, and `.containerignore` evidence and counts each signal once per provisional owner, then resolves it through `resolveContainerRootOwner` (the shared containerization adapter, see the owner-resolution rule above and [ADR 0004](adr/0004-containerization-owner-resolution.md)); no fixture/example/test path exclusions are applied, consistent with the general directory-name-exclusion policy below.
...
- **Rule (2026-08-24):** Podman/OCI, Docker, every database schema detector, and the JS/TS shared-package detector dropped their dedicated owner resolvers and resolved owners through the single shared `ownerPathForApplicationArea` in `project-structure-path-utils.ts`: `apps/<name>` or `packages/<name>` for monorepo evidence under those roots, the path up to (not including) a `src` segment for other `src`-relative evidence, and `.` otherwise. The Podman/OCI-specific deep-nested config-directory scan and the Docker-specific monorepo/config-folder contract described in earlier changelog entries and [ADR 0002](adr/0002-podman-oci-containerization-gate.md) were removed; see [Containerization and Database Owner Resolvers Removed](changelog.md#containerization-and-database-owner-resolvers-removed). **Amended 2026-09-04:** the Prisma, Drizzle, SQLAlchemy, Sequelize, and Knex database detectors regained bounded owner resolution through the engine's `ownerAdapter` (`resolveUnitRootOwner`), anchored on each tool's root-level config file -- see the owner-resolution rule in section 6.5 and [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md). **Amended 2026-09-08:** Docker moved to its own non-anchor `ownerAdapter` (`resolveContainerRootOwner`) and `ownerPathForApplicationArea` was widened -- see [ADR 0004](adr/0004-containerization-owner-resolution.md); the JS/TS shared-package detector was removed. **Amended 2026-09-10:** Podman/OCI moved onto the same `resolveContainerRootOwner` adapter (Quadlet basename matcher, `PODMAN_OCI_NON_UNIT_TOP_LEVEL_DIRECTORIES` denylist, `quadlet`/`quadlets` collection roots) -- see [ADR 0004](adr/0004-containerization-owner-resolution.md)'s 2026-09-10 update. TypeORM, Spring Boot, and Rails still use the generic resolver only.
...
- **Rule (2026-09-08):** Docker containerization owner resolution uses `resolve-container-root-owner.ts` (a non-anchor `ownerAdapter`), not the generic resolver directly: a bare root-level container file resolves to `.`; a container file one directory deep resolves to that directory unless it is a known infra/tooling/workspace-container name; deeper paths delegate to the widened `ownerPathForApplicationArea` with `products`/`plugins`/`providers` added as roots. This is not a return to the pre-2026-08-24 Docker-specific monorepo/config-folder contract -- there is no config-folder scan and no upward walk. See [ADR 0004](adr/0004-containerization-owner-resolution.md).
- **Rule (2026-09-10):** Podman/OCI containerization owner resolution uses the same `resolve-container-root-owner.ts` adapter. Its Quadlet unit (`.container`, `.pod`, `.kube`, `.build`, `.image`, `.network`, `.volume`, `.artifact`), `Containerfile`, and `.containerignore` names go through the one-directory-deep rule against `PODMAN_OCI_NON_UNIT_TOP_LEVEL_DIRECTORIES` (the Docker denylist plus `systemd`, `containers`, `.quadlet`, `quadlet`, `quadlets`, `kube`, `sysadmin`), so `n8n/n8n.container -> n8n` but `containers/systemd/x.container -> .` and `.quadlet/x.container -> .`. Deeper paths add `quadlet`/`quadlets` as collection roots, so `quadlet/valkey/valkey.container -> quadlet/valkey`. `containers/` is deliberately not a collection root (`containers/ldap/Containerfile -> .`, an `it.fails` case) because `containers/systemd/` must stay `.` and the rule is name-only. See [ADR 0004](adr/0004-containerization-owner-resolution.md)'s 2026-09-10 update.
...
- **Rule (2026-09-11):** `'CI/CD workflows'` was a placeholder in the `DetectedAreaName` union with no dispatched detector (like `'Test suite'` and `'Shared package'` before their own implementations); it is now implemented. CI/CD detected-area rules are dispatched from `detected-area-rules/ci-cd/ci-cd-area-rules.ts`, last in `applyDetectedAreaRules` (after frontend, backend, database, and containerization). Eleven single-purpose provider detectors run in a fixed precedence order -- GitHub Actions, GitLab CI/CD, CircleCI, Jenkins, Azure Pipelines, Buildkite, Travis CI, Bitbucket Pipelines, Drone/Woodpecker CI, AppVeyor, TeamCity -- so when two providers' config both resolve to the same owner, the first dispatched keeps the `primary` technology label per the general first-writer-wins rule above and the rest are carried in `related`.
- **Rule**: Every CI/CD provider except Jenkins and TeamCity gates emission on a single decisive root-config-file signal (`.github/workflows/*.yml`, `.gitlab-ci.yml`, `.circleci/config.yml`, `azure-pipelines.yml`, `.buildkite/pipeline.yml`/`buildkite.yml`, `.travis.yml`, `bitbucket-pipelines.yml`, `.drone.yml`/`.woodpecker.yml`/`.woodpecker/*`, `appveyor.yml`/`.appveyor.yml`) and resolves its owner to the repository root through the generic resolver -- no `ownerAdapter`. TeamCity gates on either its Kotlin DSL or legacy XML entrypoint under `.teamcity/`, also at the repository root. Split-config or template files (`.gitlab/ci/**`, `.circleci/src/**`, `eng/pipelines/**`, `.buildkite/pipelines/**`, TeamCity `buildTypes/`/`vcsRoots/`) are corroborating evidence only and never unlock emission alone. Jenkins is the sole exception: see the owner-resolution rule above.
- **Rule**: Deliberately undetectable CI/CD shapes, each documented as an accepted gap in its detector's own Limitations rather than treated as a bug: Azure Pipelines' per-directory `<dir>/ci.yml` multi-owner convention (the azure-sdk repos) has no reliable path-only corroboration; GitLab's project-level custom CI config path override (Settings -> CI/CD) is invisible from the tree; TeamCity's Versioned Settings and Buildkite's/AppVeyor's pipeline definitions are commonly UI-only with nothing committed to the repository.
...
- **Rule**: Django evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on `manage.py` (the project root), otherwise the generic resolver -- and scored once per signal type across `manage.py`, classic `settings.py`, split `settings/{base,local,production,test}.py`, root URL config, WSGI/ASGI entries, app config, models, migrations, admin, views, and app URL config. A Django project in a bare subdirectory or sibling Django projects in one repo resolve to their own owner instead of collapsing to the repository root.
...
- **Rule**: ASP.NET Core evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on the project's `*.csproj` (the .NET project root by SDK contract), otherwise the generic resolver -- and scored once per signal type across `.csproj` files, `Program.cs`, `Startup.cs`, server `appsettings*.json`, `Properties/launchSettings.json`, controllers, minimal endpoint folders, Razor Pages, MVC views, and `wwwroot`. `wwwroot/appsettings*.json` is treated as client config rather than server appsettings proof. Non-web `.csproj` owners (class libraries, test projects) collect only the project-file signal and never pass the gate; multi-project solutions and sibling web hosts split by owner instead of collapsing to `.`.
...
- **Rule**: Laravel evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on `artisan` (the Laravel project root), otherwise the generic resolver -- and scored once per signal type across `artisan`, `bootstrap/app.php`, modern bootstrap providers, application and route service providers, the legacy HTTP kernel, standard route files, the public entry, controllers, Blade views, models, migrations, seeders, `config/app.php`, and Composer manifests.
...
- **Rule (2026-08-24, amended 2026-09-04):** The 2026-08-24 consolidation removed the per-database owner resolvers (see [Containerization and Database Owner Resolvers Removed](changelog.md#containerization-and-database-owner-resolvers-removed)). On 2026-09-04 the Prisma, Drizzle, SQLAlchemy, Sequelize, and Knex detectors regained bounded owner resolution through the engine's `ownerAdapter` (`resolveUnitRootOwner`), anchored on each tool's root-level config file (`prisma.config.*`, `drizzle.config.*`, `alembic.ini`, `.sequelizerc`, `knexfile.*`); see the owner-resolution rule above and [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md). TypeORM still resolves owners through the shared generic `ownerPathForApplicationArea`, the same resolver every non-anchored detector falls back to. An anchored database detector emits at the config file's directory (e.g. `apps/backend` for `apps/backend/prisma.config.ts`), and a schema in a bare subdirectory such as `backend/prisma/` no longer collapses to `.`; a non-anchored detector still emits at the broader generic application owner.
- **Rule**: Prisma evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on a root-level `prisma.config.*` (the `.config/prisma.*` variant stays a non-anchor match and only feeds the gate and score), otherwise the generic resolver -- and scored once per signal type across `schema.prisma`, `.prisma` schema fragments under `prisma/`, `prisma.config.*` or `.config/prisma.*`, `prisma/migrations`, Prisma `migration.sql` files, and `migration_lock.toml`.
...
- **Rule**: Drizzle evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on `drizzle.config.*` (or a `drizzle-<name>.config.*` variant), otherwise the generic resolver -- even though Drizzle schema, SQL migrations, journal, and snapshots may live in separate folders below that config.
...
- **Rule**: Knex evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on `knexfile.*` (or a `_knexfile`/`knex.config`/`knexfile.<env>` variant), otherwise the generic resolver -- even though migration and seed files can be configured into separate locations below that config. The gate always requires a config signal, so a Knex owner never emits without its anchor present.
...
- **Rule**: SQLAlchemy evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on `alembic.ini`, otherwise the generic resolver. A single repository-root `alembic.ini` that fans out to several services via `version_locations` anchors the repository root, whose `.` owner does not claim the sub-service paths, so those still fall back to the generic resolver; a per-service `alembic.ini` resolves each service to its own owner.
...
- **Rule**: TypeORM evidence is grouped by the generic application owner (see the 2026-08-24 owner-resolution rule above), not a dedicated database-folder-aware owner, even though TypeORM config, data-source files, entities, and migrations often live across sibling `src` folders. Unlike the other database detectors, TypeORM keeps the generic resolver: `ormconfig.*` is deprecated and being removed, and `data-source.ts` has no contract location (the CLI requires an explicit `-d` path), so there is no reliable root anchor.
...
- **Rule**: Sequelize evidence is grouped by owner -- resolved through `resolve-unit-root-owner` anchored on `.sequelizerc`, otherwise the generic resolver. sequelize-cli places `config/`, `models/`, `migrations/`, and `seeders/` at the project root rather than under `src/`, so without the anchor the generic resolver has no boundary to group them by; when no `.sequelizerc` is committed, the config-plus-model gate branches still apply through the generic resolver.
...
- **Rule**: Broad frontend detectors declare strong competing-framework evidence inline as the declarative engine's `competingProofSchemas` (e.g. React vetoes owners carrying Next.js or React Router proof, Vue vetoes owners carrying Nuxt proof); those schemas match only distinctive framework files, not weak support files.
...
- **Rule**: SvelteKit frontend area evidence is grouped by owner path (resolved through `resolve-unit-root-owner`, anchored on `svelte.config.*`) and scored once per signal type: shared `svelte.config.*` support, strong `+page.svelte`/`+layout.svelte` route components, support `+page.*`/`+layout.*` load files, `+server.*` routes, `src/app.html`, and weak `src/routes` hints; SvelteKit areas emit only from route components or load/server evidence paired with the app template because standalone Svelte projects may also contain `svelte.config.*`. The gate still rejects a standalone Svelte project even when `svelte.config.*` anchors an owner, so anchoring on a non-SvelteKit-exclusive file does not cause false emission.
...
- **Rule**: Astro frontend area evidence is grouped by owner path (resolved through `resolve-unit-root-owner`, anchored on `astro.config.*`) and scored once per signal type: `astro.config.*`, strong `src/pages/*.astro` page files, weak `src/pages/*.{md,mdx,html}` content page hints, endpoint files, support `src/layouts/*.astro`/`src/components/*.astro`, and weak `src/pages` hints; Astro areas emit only from config or `.astro` page proof.
...
- [ ] ~~Shared package detector path rules~~ removed 2026-09-08 (low-signal output); see changelog
- [x] Docker containerization detector path rules
- [x] Podman/OCI containerization detector path rules (gate implemented; owner resolution uses the `resolveContainerRootOwner` adapter, see [ADR 0004](adr/0004-containerization-owner-resolution.md) and the 2026-09-10 changelog entry)
- [x] CI/CD workflows detector path rules (11 providers: GitHub Actions, GitLab CI/CD, CircleCI, Jenkins, Azure Pipelines, Buildkite, Travis CI, Bitbucket Pipelines, Drone CI/Woodpecker CI, AppVeyor, TeamCity; `'CI/CD workflows'` was a placeholder area name until this 2026-09-11 implementation)
- [ ] Generic backend fallback path rules
- [x] Declarative area-rule engine for framework detectors (every framework, database, containerization, and CI/CD detector is declarative after the ASP.NET Core (2026-08-29) and NestJS (2026-09-04) conversions; gate, nested `and`/`or` condition nodes, competing-proof, shared-candidate-map veto, and dynamic related-tech support, see the 2026-08-27 through 2026-09-04 changelog entries); `evaluateCondition` shape evaluation has direct unit coverage (`declarative-area-rule-engine.test.ts`)
- [x] Per-shape owner adapters for the declarative engine (`ownerAdapter` param, anchor-signal two-pass, `resolveUnitRootOwner` wired into fifteen detectors -- Next.js, Nuxt, Angular, Astro, React Router, SvelteKit, Django, ASP.NET Core, Laravel, Prisma, Drizzle, SQLAlchemy, Sequelize, Knex, Jenkins; see [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md) and the 2026-09-04 and 2026-09-11 changelog entries). The Docker and Podman/OCI detectors share a non-anchor `resolveContainerRootOwner` adapter -- see [ADR 0004](adr/0004-containerization-owner-resolution.md) and the 2026-09-08 and 2026-09-10 changelog entries.
...
| Consolidated generic owner resolver (2026-08-24) is less precise than the removed per-technology resolvers, so schema/containerization areas may group under a broader application owner than the tool's own root. The Prisma, Drizzle, SQLAlchemy, Sequelize, Knex, Django, ASP.NET Core, Laravel, and config-anchored frontend detectors regained config-anchored precision on 2026-09-04 (ADR 0003); Docker gained the `resolveContainerRootOwner` adapter on 2026-09-08 and Podman/OCI on 2026-09-10 (ADR 0004). The risk remains for TypeORM, Spring Boot, and Rails. | Revisit `project-structure-path-utils.ts` if repository evidence shows the generic resolver misattributing ownership for a still-unanchored detector.                                  |
| `resolveContainerRootOwner`'s one-directory-deep rule is a heuristic: a single app split into sibling `frontend/` + `backend/` folders resolves to two `Containerization` owners, and a versioned nested sub-service (`apps/api/v2`) collapses to its parent. `NON_UNIT_TOP_LEVEL_DIRECTORIES`, `PODMAN_OCI_NON_UNIT_TOP_LEVEL_DIRECTORIES`, and the widened `MONOREPO_OWNER_ROOT_DIRECTORIES` are hand-maintained lists that must track `project-shape-detector.ts`. On the Podman/OCI side `containers/<name>/Containerfile` collapses to `.` because `containers/` cannot be a collection root without breaking `containers/systemd/`. | Unit coverage in `resolve-container-root-owner.test.ts` (36-repo scan) and `resolve-container-root-owner.podman-oci.test.ts` (~25-repo Quadlet/OCI scan) pins each known gap as `it.fails`; move to manifest-aware detection if false splits are observed. |
| `project-structure-analyzer.test.ts` was deleted, removing the end-to-end coverage that asserted `(area, owner)` output across detectors; only `project-structure-detected-area-candidates.ts`, `resolve-unit-root-owner.test.ts`, `resolve-container-root-owner.test.ts`, `resolve-container-root-owner.podman-oci.test.ts`, and `declarative-area-rule-engine.test.ts` (`evaluateCondition`) retain unit coverage. `project-structure-path-utils.test.ts` is still an empty stub, so the 2026-09-08 widening of `ownerPathForApplicationArea` (`@scope` depth, workspace-root array, `extraRootDirectories`) has only indirect coverage through the two `resolve-container-root-owner` specs. | Rebuild analyzer-level fixtures per detector before further behavior changes; add direct `ownerPathForApplicationArea` unit coverage. `resolve-unit-root-owner`/`resolve-container-root-owner` and `evaluateCondition` are unit-covered, but no analyzer-output fixture exercises the two-pass or containerization owner path end to end. |
