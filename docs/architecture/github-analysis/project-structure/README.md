...
- **Rule**: `DetectedAreaTechnology` is composed from frontend, backend, containerization, runtime/platform, template, and database technology unions in `project-structure-analyzer.types.ts`; keep the public union stable while adding new labels to the narrow category that owns them.
...
- **Rule**: Containerization detected-area rules are dispatched from `detected-area-rules/containerization/containerization-area-rules.ts`. Runtime-specific containerization detectors live in separate modules: Docker and the Podman/OCI combination gate are implemented. Podman/OCI collects basename-only Quadlet unit, `Containerfile`, and `.containerignore` evidence and counts each signal once per provisional application owner, then resolves it through `ownerPathForPodmanOciContainerizationArea`; no fixture/example/test path exclusions are applied, consistent with the general directory-name-exclusion policy below.
...
- **Rule**: Podman/OCI owner resolution reuses the same monorepo-root/member contract as Docker for `apps/*`, `services/*`, `packages/*`, and `libs/*` owners, then scans the full remaining path -- not only the top-level segment -- for the first occurrence of a recognized Podman/OCI config directory (`quadlet`, `containers`, `systemd`, `deploy`, `etc`) and collapses the owner to everything before it, so deeply nested layouts such as `subsystems/video/etc/containers/systemd/*.container` resolve to `subsystems/video` instead of the config mirror folder. Root-level config-directory or single-segment evidence resolves to `.`; evidence with no recognized monorepo or config-directory match falls back to its local parent folder. See `ownerPathForPodmanOciContainerizationArea` in `project-structure-path-utils.ts`.
- **Rule**: Podman/OCI detector confidence coverage includes the coherent-combination emission and weak-shape rejection fixtures through the public analyzer output, plus monorepo-member, monorepo-root, root-level-config-directory, deep-nested-config-directory, and owner-isolation fixtures through both the public analyzer output and dedicated `ownerPathForPodmanOciContainerizationArea` unit coverage in `project-structure-path-utils.test.ts`.
...
- [x] Podman/OCI containerization detector path rules (gate and owner resolution implemented)
