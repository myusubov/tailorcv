...
- **Rule**: The declarative engine (`detected-area-rules/declarative-area-rule-engine.ts`) resolves every signal's owner through the generic `ownerPathForApplicationArea` unless the detector passes an `ownerAdapter`. With an anchor-based adapter, the engine evaluates anchor schemas (`isAnchorSignal: true`) first, collects each anchor signal's resolved owner into an `anchorOwners` set, then resolves non-anchor signals against that completed set in a second pass; a detector with no anchor schema just has its adapter called once per matched entry. Two adapters exist:
  ...
  - `detected-area-rules/owner-adapters/resolve-container-root-owner.ts`, passed by the Docker detector with no anchor schema. A bare root-level file resolves to `.`; a container file one directory deep resolves to that directory unless the directory is a known infra/tooling/workspace-container name (`NON_UNIT_TOP_LEVEL_DIRECTORIES`), in which case `.`; every deeper path delegates to `ownerPathForApplicationArea` with `products`/`plugins`/`providers` added as workspace roots for that call. See [ADR 0004](adr/0004-containerization-owner-resolution.md).
...
- [x] Per-shape owner adapters for the declarative engine (`ownerAdapter` param, anchor-signal two-pass, `resolveUnitRootOwner` wired into fourteen detectors -- Next.js, Nuxt, Angular, Astro, React Router, SvelteKit, Django, ASP.NET Core, Laravel, Prisma, Drizzle, SQLAlchemy, Sequelize, Knex; see [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md) and the 2026-09-04 changelog entry). Docker uses a non-anchor `resolveContainerRootOwner` adapter -- see [ADR 0004](adr/0004-containerization-owner-resolution.md) and the 2026-09-08 changelog entry.
...
| Consolidated generic owner resolver (2026-08-24) is less precise than the removed per-technology resolvers, so schema/containerization areas may group under a broader application owner than the tool's own root. The Prisma, Drizzle, SQLAlchemy, Sequelize, Knex, Django, ASP.NET Core, Laravel, and config-anchored frontend detectors regained config-anchored precision on 2026-09-04 (ADR 0003); Docker gained the `resolveContainerRootOwner` adapter on 2026-09-08 (ADR 0004). The risk remains for TypeORM, Spring Boot, Rails, and Podman/OCI. | Revisit `project-structure-path-utils.ts` if repository evidence shows the generic resolver misattributing ownership for a still-unanchored detector.                                  |
| `resolveContainerRootOwner`'s one-directory-deep rule is a heuristic: a single app split into sibling `frontend/` + `backend/` folders resolves to two `Containerization` owners, and a versioned nested sub-service (`apps/api/v2`) collapses to its parent. `NON_UNIT_TOP_LEVEL_DIRECTORIES` and the widened `MONOREPO_OWNER_ROOT_DIRECTORIES` are hand-maintained lists that must track `project-shape-detector.ts`. | Unit coverage in `resolve-container-root-owner.test.ts` derives cases from a 36-repo scan and pins the known gap as `it.fails`; move to manifest-aware detection if false splits are observed. |
| `project-structure-analyzer.test.ts` was deleted, removing the end-to-end coverage that asserted `(area, owner)` output across detectors; only `project-structure-detected-area-candidates.ts`, `resolve-unit-root-owner.test.ts`, `resolve-container-root-owner.test.ts`, and `declarative-area-rule-engine.test.ts` (`evaluateCondition`) retain unit coverage. `project-structure-path-utils.test.ts` is still an empty stub, so the 2026-09-08 widening of `ownerPathForApplicationArea` (`@scope` depth, workspace-root array, `extraRootDirectories`) has only indirect coverage through `resolve-container-root-owner.test.ts`. | Rebuild analyzer-level fixtures per detector before further behavior changes; add direct `ownerPathForApplicationArea` unit coverage. `resolve-unit-root-owner`/`resolve-container-root-owner` and `evaluateCondition` are unit-covered, but no analyzer-output fixture exercises the two-pass or containerization owner path end to end. |
