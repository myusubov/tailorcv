...
| `apps/backend/src/services/github-analysis/project-structure/detected-area-rules/declarative-area-rule-engine.ts` | Generic schema-driven detector runner used by every framework, database, containerization, and shared-package detector (the Jest detector was removed with the Test-suite category; the ASP.NET Core (2026-08-29) and NestJS (2026-09-04) conversions leave none hand-written); supports `gateBlocker` shape checks with nested `and`/`or` condition nodes, `competingProofSchemas` file-evidence vetoes, a `checkForExistingCandidate` shared-candidate-map veto, a `dynamicRelatedTechMap` per-signal related-technology union (see the 2026-08-27, 2026-08-28, and 2026-08-29 changelog entries), and an optional `ownerAdapter` with anchor-signal two-pass owner resolution. | Changing shared detector matching/gating/veto behavior |
| `apps/backend/src/services/github-analysis/project-structure/detected-area-rules/owner-adapters/`                 | Per-framework-shape owner resolvers passed to the engine as `ownerAdapter`; `resolve-unit-root-owner.ts` anchors on the framework or tool config file at the unit root and is shared by fourteen detectors (Next.js, Nuxt, Angular, Astro, React Router, SvelteKit, Django, ASP.NET Core, Laravel, Prisma, Drizzle, SQLAlchemy, Sequelize, Knex) -- see [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md).                                                                                                                                                                                                                                                                          | Adding an owner adapter for a new framework shape      |
...
- [x] Declarative area-rule engine for framework detectors (every framework, database, containerization, and shared-package detector is declarative after the ASP.NET Core (2026-08-29) and NestJS (2026-09-04) conversions; gate, nested `and`/`or` condition nodes, competing-proof, shared-candidate-map veto, and dynamic related-tech support, see the 2026-08-27 through 2026-09-04 changelog entries); `evaluateCondition` shape evaluation has direct unit coverage (`declarative-area-rule-engine.test.ts`)
- [x] Per-framework owner adapters for the declarative engine (`ownerAdapter` param, anchor-signal two-pass, `resolveUnitRootOwner` wired into fourteen detectors -- Next.js, Nuxt, Angular, Astro, React Router, SvelteKit, Django, ASP.NET Core, Laravel, Prisma, Drizzle, SQLAlchemy, Sequelize, Knex; see [ADR 0003](adr/0003-pluggable-owner-adapters-anchor-signals.md) and the 2026-09-04 changelog entry)
...
| Consolidated generic owner resolver (2026-08-24) is less precise than the removed per-technology resolvers, so schema/containerization areas may group under a broader application owner than the tool's own root. The Prisma, Drizzle, SQLAlchemy, Sequelize, Knex, Django, ASP.NET Core, Laravel, and config-anchored frontend detectors regained config-anchored precision on 2026-09-04 (ADR 0003); the risk remains for TypeORM, Spring Boot, Rails, Docker, Podman/OCI, and the shared-package detector | Revisit `project-structure-path-utils.ts` if repository evidence shows the generic resolver misattributing ownership for a still-unanchored detector.                                  |
| `project-structure-analyzer.test.ts` was deleted, removing the end-to-end coverage that asserted `(area, owner)` output across detectors; only `project-structure-detected-area-candidates.ts`, `project-structure-path-utils.ts` (currently an empty stub), `resolve-unit-root-owner.test.ts`, and `declarative-area-rule-engine.test.ts` (`evaluateCondition`) retain unit coverage | Rebuild analyzer-level fixtures per detector before further behavior changes; `resolve-unit-root-owner` two-pass owner resolution and `evaluateCondition` shape evaluation are unit-covered, but fourteen detectors now depend on the two-pass path and no analyzer-output fixture exercises it end to end. |
