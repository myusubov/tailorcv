...
| `apps/backend/src/services/github-analysis/project-structure/detected-area-rules/declarative-area-rule-engine.ts` | Generic schema-driven detector runner used by 20 of the framework, database, containerization, and test detectors; supports `gateBlocker` shape checks, `competingProofSchemas` vetoes, and a `dynamicRelatedTechMap` per-signal related-technology union (see the 2026-08-27 and 2026-08-28 changelog entries). Four detectors remain permanently hand-written -- see Risks & Mitigations below. | Changing shared detector matching/gating/veto behavior |
...
- [x] Declarative area-rule engine for framework detectors (20 detectors migrated across three waves with gate, competing-proof, and dynamic related-tech support; ASP.NET Core, React, Static frontend, and Express remain permanently hand-written, see the 2026-08-27 and 2026-08-28 changelog entries)
...
| Four detectors (ASP.NET Core, React, Static frontend, Express) cannot be expressed in the declarative schema as it exists today -- gating on the shared candidate map instead of raw file evidence, a per-entry exclusion predicate, and a two-independent-OR-groups condition shape in one gate node are all unsupported | Keep these detectors hand-written; only extend `applyDeclarativeAreaDetector` if a concrete need arises, see the 2026-08-27 and 2026-08-28 changelog entries. |
