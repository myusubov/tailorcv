...
- **2026-09-10 amendment:** owner resolution moved off the generic resolver again. The detector now passes `resolveContainerRootOwner` (the Docker containerization adapter, [ADR 0004](0004-containerization-owner-resolution.md)), extended with a Quadlet/`Containerfile` basename matcher and the `PODMAN_OCI_NON_UNIT_TOP_LEVEL_DIRECTORIES` denylist; `quadlet/` and `quadlets/` are treated as collection roots. This is not the deep-nested config-directory scan removed in 2026-08-24 -- it is a one-directory-deep rule plus a fixed denylist. The combination-gate decision is still unaffected. See [Podman/OCI Containerization Owner Resolution via `resolveContainerRootOwner`](../changelog.md#podmanoci-containerization-owner-resolution-via-resolvecontainerrootowner).
