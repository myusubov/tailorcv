...
- Use HeroUI's `secondary` field variants when OTP slots or password inputs sit directly on a `surface` Card so their `default` background remains distinguishable in light and dark themes without changing global field tokens.
...
- [x] Reset-code `InputOTP` slots and new-password inputs remain visible on Card surfaces in light and dark themes
