    ...
    try {
        for await (const rawEvent of openaiStream) {
            ...
            if (
                ...
                isToolCall = true;
                if (!hasYieldedStart) {
                     yield { type: 'text', content: 'Generating resume edit...' };
        ...
        if (isToolCall) {
            try {
                ...
                if (args?.proposal && args?.explanation) {
                    ...
                    yield { type: 'text', content: "The edit couldn't be processed - please try rephrasing your request." };
                }
            } catch (error) {
                logger.error({ error, toolCallArgs }, 'Failed to parse tool call arguments');
                yield { type: 'text', content: "I encountered an error processing that edit." };
            }
        }
